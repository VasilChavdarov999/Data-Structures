...
public class WrongMeasurements {
    ...
    private static int getValidValue(int[][] matrix, int row, int col, int wrongValue) {
        int sum = 0;
        if (isInRangeOfMatrix(row + 1, col, matrix) && matrix[row + 1][col] != wrongValue){
            sum += matrix[row + 1][col];
        }
        if (isInRangeOfMatrix(row - 1, col, matrix) && matrix[row - 1][col] != wrongValue){
            sum += matrix[row - 1][col];
        }
        if (isInRangeOfMatrix(row, col + 1, matrix) && matrix[row][col + 1] != wrongValue){
    ...
    private static boolean isInRangeOfMatrix(int row, int col, int[][] matrix){
